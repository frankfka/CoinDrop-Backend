...
async function getSupportedCurrencies() {
  const config = await BackendConfig.findOne({ version: configVersion })
...
async function getCoinDetails(coins) {
  ...
  const cached = cacheService.getCoinInfo(coins); // Map of tickers to cached objects
  let retrievedFromApi = []; // Array of coin info objects from API
  const toGetFromApi = coins.filter(currencyCode => !cached.has(currencyCode));
